...
export const metadata = createMetadata({
	title: "EgiftcardContract.Write | Egiftcard .NET SDK",
	description: "Execute state-changing transactions on a smart contract.",
});

# EgiftcardContract.Write

The `EgiftcardContract.Write` method allows you to execute transactions that alter the state of the blockchain via a smart contract. This is typically used for functions that create, modify, or delete data on the blockchain.

## Usage

```csharp
var transactionReceipt = await EgiftcardContract.Write(wallet, contract, "methodName", weiValue, parameters);
```
...
An instance of `IEgiftcardWallet`. This represents the signer of the transaction, which can be any type of wallet provider.

### contract (required)

An instance of `EgiftcardContract`. Represents the smart contract you wish to interact with.
...
Let's say you have a smart contract with a `transfer` method that transfers ERC20 tokens from the caller's address to another address. Here's how you could execute a transaction to transfer tokens using `EgiftcardContract.Write`:
...
var contract = await EgiftcardContract.Create(client, contractAddress, chainId);
...
var receipt = await EgiftcardContract.Write(wallet, contract, "transfer", weiValue, toAddress, amount);
