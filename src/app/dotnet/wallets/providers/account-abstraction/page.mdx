...
export const metadata = createMetadata({
	title: "SmartWallet.Create | Egiftcard .NET SDK",
	description:
...
An instance of `EgiftcardClient`. Facilitates communication with the blockchain.

### personalWallet (required)

An instance of IEgiftcardWallet (e.g., `PrivateKeyWallet` or `InAppWallet`). Represents the personal wallet connected to the SmartWallet.
...
Here's how you can create a SmartWallet for a user, assuming you already have a EgiftcardClient and a personal wallet (PrivateKeyWallet or InAppWallet) set up:
