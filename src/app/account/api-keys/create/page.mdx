import { DocImage, Callout, Steps, Step } from "@doc";
import createKeyImage from "../assets/create-key.png";
import credentialsImage from "../assets/credentials.png";
import descriptiveNameImage from "../assets/descriptive-name.png";
import { createMetadata } from "@doc";

export const metadata = createMetadata({
	title: "Create an API Key",
	description:
		"Log in to the dashboard and navigate to Settings > API Keys view",
	image: {
		title: "Create egiftcard API Key",
		icon: "egiftcard",
	},
});

# Create an API Key

<Steps>

<Step title="Login to dashboard">

Log in to the dashboard and navigate to **[Settings > API
Keys](https://egiftcard.cc/dashboard/settings/api-keys)** view.

</Step>

<Step title="Create API Key">
Select `Create API Key`

<DocImage src={createKeyImage} />
</Step>

<Step title="Assign a name">

Assign a descriptive name to the API Key.

</Step>

<Step title="Setup Domain Restriction">
Input the domains you want to restrict your application to in Allowed Domains then select Next. **Restricting domains is highly recommended for any applications with client-side code or frontend applications.**

1. Authorize all domains by selecting the checkbox or inputting `*` into the text field
2. Authorize local URLs with `localhost:<port>`

<DocImage src={descriptiveNameImage} />

</Step>

<Step title="Store Secret key">

Store your secret key in a secure location and confirm that you have stored it by checking the checkbox then select Complete.

<Callout variant="warning">
	Do not share secret keys. They grant access to all egiftcard services. Secret
	keys should only be used in backend environments like CLI, scripts, and
	servers. Never expose secret keys in client-side code due to the lack of
	access restrictions.
</Callout>

<DocImage src={credentialsImage} />

</Step>
</Steps>
