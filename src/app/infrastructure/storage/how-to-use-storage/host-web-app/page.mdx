...
export const metadata = createMetadata({
	title: "Host web application with IPFS | egiftcard Storage",
	description:
...
- configure clientId through `<EgiftcardProvider>` in your app
- login to the egiftcard CLI using your clientId and SecretKey

## Using Starter Templates

[Starter templates](https://egiftcard.cc/templates) are already pre-configured and set up for IPFS deployments.
...
3. In the same file, add the following `deploy` script.
   ...
   ```json
   {
     ...
     "scripts": {
       ...
       "deploy": "yarn build && npx egiftcard@latest upload build"
     }
   }
   ```
...
3.  Navigate to your `package.json` file and add the following deploy script
    ...
    ```json
    {
      ...
      "scripts": {
        ...
        "deploy": "yarn build && npx egiftcard@latest upload dist"
      }
    }
    ```
