import { Callout } from "@doc";
import { GithubButtonLink } from "@doc";
import { createMetadata } from "@doc";

export const metadata = createMetadata({
	image: {
		title: "ERC1155Burnable contract",
		icon: "solidity",
	},
	title: "ERC1155Burnable | egiftcard contract",
	description:
		"ERC1155Burnable allows the NFTs in the contract to be burned (transferred to a non-recoverable address)",
});

# ERC1155Burnable

```solidity
import "@egiftcard/contracts/extension/interface/IBurnableERC721.sol";
```

ERC1155Burnable allows the NFTs in the contract to be burned (transferred to a non-recoverable address).

<Callout title="info" variant="info">

This extension is an interface and requires all of the functions to be implemented

</Callout>

<br />

<GithubButtonLink href="https://github.com/egiftcard/contracts/blob/main/contracts/extension/interface/IBurnableERC1155.sol" />

## Usage

This is an example smart contract demonstrating how to inherit from this extension and override the functions to add (optional) custom functionality.

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@egiftcard/contracts/eip/ERC1155.sol";
import "@egiftcard/contracts/extension/interface/IBurnableERC1155.sol";

contract Contract is ERC1155, IBurnableERC1155 {
    constructor(
        string memory _name,
        string memory _symbol
    )
        ERC1155(
            _name,
            _symbol
        )
    {}

    function burn(address account, uint256 id, uint256 value) external override {
        // Your custom implementation here
    }

    function burnBatch(
        address account, uint256[] memory ids, uint256[] memory values) external override {
        // Your custom implementation here
    }
}
```

## Base Contracts Implementing This Extension

- [ERC1155Base](/contracts/build/base-contracts/erc-1155/base)
- [ERC1155DelayedReveal](/contracts/build/base-contracts/erc-1155/delayed-reveal)
- [ERC1155Drop](/contracts/build/base-contracts/erc-1155/drop)
- [ERC1155LazyMint](/contracts/build/base-contracts/erc-1155/lazy-mint)
- [ERC1155SignatureMint](/contracts/build/base-contracts/erc-1155/signature-mint)
