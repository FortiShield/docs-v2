import { Callout } from "@doc";
import { GithubButtonLink } from "@doc";
import { createMetadata } from "@doc";

export const metadata = createMetadata({
	image: {
		title: "ERC721Burnable contract",
		icon: "solidity",
	},
	title: "ERC721Burnable | egiftcard contract",
	description:
		"ERC721Burnable allows the NFTs in the contract to be burned (transferred to a non-recoverable address)",
});

# ERC721Burnable

```solidity
import "@egiftcard/contracts/extension/interface/IBurnableERC721.sol";
```

ERC721Burnable allows the NFTs in the contract to be burned (transferred to a non-recoverable address).

<Callout title="info" variant="info">

This extension is an interface and requires all of the functions to be implemented

</Callout>

<br />

<GithubButtonLink href="https://github.com/egiftcard/contracts/blob/main/contracts/extension/interface/IBurnableERC721.sol" />

## Usage

This is an example smart contract demonstrating how to inherit from this extension and override the functions to add (optional) custom functionality.

```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@egiftcard/contracts/eip/ERC721A.sol";
import "@egiftcard/contracts/extension/interface/IBurnableERC721.sol";

contract Contract is ERC721A, IBurnableERC721 {
    constructor(
        string memory _name,
        string memory _symbol
    )
        ERC721A(
            _name,
            _symbol
        )
    {}

    function burn(uint256 tokenId) external {
        // Your custom implementation here
    }
}
```

## Base Contracts Implementing This Extension

- [ERC721Base](/contracts/build/base-contracts/erc-721/base)
- [ERC721DelayedReveal](/contracts/build/base-contracts/erc-721/delayed-reveal)
- [ERC721Drop](/contracts/build/base-contracts/erc-721/drop)
- [ERC721LazyMint](/contracts/build/base-contracts/erc-721/lazy-mint)
- [ERC721SignatureMint](/contracts/build/base-contracts/erc-721/signature-mint)
