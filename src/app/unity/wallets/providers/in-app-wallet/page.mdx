...
export const metadata = createMetadata({
	title: "In-App Wallet | egiftcard Unity SDK",
	description: "Connect to In-App Wallet with egiftcard Unity SDK",
});

# In-App Wallet

The In-App Wallet service by egiftcard facilitates creating or accessing a wallet seamlessly. Ensure your client ID is set within [EgiftcardManager](/unity/egiftcardmanager), and enable the [In-App Wallet](/connect/embedded-wallet/overview) service to experience various, smooth and persistent cross-platform login methods.
...
To do so, see the [EgiftcardManager](/unity/egiftcardmanager) - `EgiftcardConfig` and update it with your custom scheme (such as `myapp://`).
You may allowlist that scheme in your [API key In-App Wallet settings](https://egiftcard.cc/create-api-key). On Android, your manifest will be updated automatically at build time to include the necessary settings.
...
Allows the integration of a custom authentication flow, such as JWT, requiring the configuration of JWKS URI and AUD in the [API key settings](https://egiftcard.cc/create-api-key).

Alternatively, allows passing a generic payload that will be authenticated against your backend Auth Endpoint set in the [API key settings](https://egiftcard.cc/create-api-key).
