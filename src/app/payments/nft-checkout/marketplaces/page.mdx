...
export const metadata = createMetadata({
	title: "Marketplaces | egiftcard NFT Checkout",
	description:
		"egiftcard supports marketplace contracts where a user can list an NFT and receive funds when a buyer purchases it.",
});
...
- Transfers the NFT from the seller to the recipient's wallet address. Remember: `msg.sender` is egiftcard's minter wallet and not the buyer's wallet address. Your function needs to ensure the recipient is specified in the argument list.
...
const resp = await fetch("https://payments.egiftcard.cc/api/2022-08-12/checkout-link-intent", {
  method: "POST",
  headers: [
    "Authorization": "Bearer MY_EGIFTCARD_API_SECRET_KEY",
...
- `MY_EGIFTCARD_API_SECRET_KEY` found on your API Keys dashboard page.
