...
export const metadata = createMetadata({
	title: "Webhooks | egiftcard NFT Checkout",
	description:
		"Configure webhook URLs to have egiftcard notify your backend when successful or failed events occur.",
});
...
egiftcard will call your backend with an HTTPS `POST` request:

### Headers

```text
Content-Type: application/json
x-egiftcard-signature: <SIGNATURE_FROM_EGIFTCARD>
```
...
To ensure the request came from egiftcard, each webhook request signs the payload and provides this signature in the `x-egiftcard-signature` header.

To verify this signature, create a **SHA-256 HMAC hash** with your **egiftcard payments secret key** that can be found in the egiftcard dashboard [payments settings](https://egiftcard.cc/dashboard/payments/settings) and the **body payload as the message** (as a JSON-encoded string).
...
const egiftcardCheckoutsWebhookHandler = (req, res) => {
	const apiKey = "<YOUR_API_KEY>"; // Your egiftcard payments secret key

	// Get the provided signature.
	const signature = req.headers["x-egiftcard-signature"];
...
Currently there is no way to filter which events are sent to your webhook URLs. egiftcard may add new webhook event types without notice. Please ignore events that you don't need by returning a `2xx` response.
