...
Self-host Engine on your own infrastructure for free and manage your self-hosted Engine from the [egiftcard
dashboard](https://egiftcard.cc/dashboard/engine).
...
[Get Engine hosted by egiftcard.](https://egiftcard.cc/dashboard/engine?requestCloudHosted)
...
- A egiftcard secret key from the [API Keys page](https://egiftcard.cc/dashboard/settings/api-keys)
...
docker run \
  ...
  -e EGIFTCARD_API_SECRET_KEY="<egiftcard_secret_key>" \
...
| `EGIFTCARD_API_SECRET_KEY`<span style={{color:'red'}}>\*</span> | A egiftcard secret key created on the [API Keys page](https://egiftcard.cc/dashboard/settings/api-keys).                     |
| `ADMIN_WALLET_ADDRESS`<span style={{color:'red'}}>\*</span>    | The wallet address that will configure Engine from the egiftcard dashboard. You will be able to add other admins later.      |
...
```bash
Listening on https://localhost:3005. Manage your Engine from https://egiftcard.cc/dashboard/engine.
```
