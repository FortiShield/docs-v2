...
The egiftcard SDK works with **any EVM chain**.

All you need to connect a chain is its chain id. RPC connection to the chain is handled for you.

```ts
import { defineChain } from "egiftcard";

const myChain = defineChain(myChainId);
```

The SDK comes with predefined [popular chains](/references/typescript/v5/variables) like `base`, `polygon`, and more exported from the `egiftcard/chains` entrypoint.
