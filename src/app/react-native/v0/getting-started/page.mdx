...
export const metadata = createMetadata({
	image: {
		title: "Getting Started with egiftcard React Native SDK",
		icon: "react",
	},
	title: "Getting Started with egiftcard React Native SDK",
	description: "Getting Started with egiftcard React Native SDK",
});

# Getting Started

Our SDK uses a [Provider Pattern](https://flexiple.com/react/provider-pattern-with-react-context-api/), meaning any component within the `EgiftcardProvider` will have access to the SDK.

<Callout variant='info' title="API Key">

You will require an API key to use egiftcard's infrastructure services with the SDK.

you need to first obtain an API key from the [dashboard](https://egiftcard.cc/dashboard) and then copy the "Client ID" and pass it to the `EgiftcardPovider` as the `clientId` prop.

</Callout>

Let's take a look at a typical setup.

## Configure the EgiftcardProvider
...
const App = () => {
	return (
		<EgiftcardProvider activeChain={"mainnet"} clientId={"your-client-id"}>
			<AppInner />
		</EgiftcardProvider>
	);
};
...
<p>
	[Create React Native
	App](https://github.com/egiftcard-example/react-native-typescript-starter/blob/main/App.tsx)
</p>
...
import { ConnectWallet } from "@egiftcard/react-native";
...
import { useContract } from "@egiftcard/react-native";
...
```jsx
import { useContract, useNFTs } from "@egiftcard/react-native";
...
```jsx
import { useContract, useNFTs, useMintNFT } from "@egiftcard/react-native";
...
The `EgiftcardProvider` offers a number of configuration options to control the behavior of the React Native and Typescript SDK.
We provide defaults for all of them, but you customize them to suit your needs. For more info see the [EgiftcardProvider reference](/references/react-native/v0/EgiftcardProvider).
...
You can pass a `locale` prop to the `EgiftcardProvider` with the strings in the language of your preference. See the list of strings to translate [here](https://github.com/egiftcard/js/blob/main/packages/react-native/src/evm/i18n/en.ts).
...
```jsx
import { EgiftcardProvider, en } from "@egiftcard/react-native";
import React from "react";
