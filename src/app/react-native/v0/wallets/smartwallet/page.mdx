...
export const metadata = createMetadata({
	...
	title: "Smart Wallet | egiftcard React Native SDK",
...
```tsx
import { smartWallet, metamaskWallet } from "@egiftcard/react-native";
...
To allow users to connect to this wallet using the [ConnectWallet](/references/react-native/v0/ConnectWallet) component, you can add it to [EgiftcardProvider's supportedWallets](/react-native/v0/connecting-wallets#supported-wallets) prop.
...
`smartWallet()` also needs to be added in [EgiftcardProvider's supportedWallets](/react-native/v0/connecting-wallets#supported-wallets) if you want the wallet to auto-connect on next page load.

```tsx
import { useSmartWallet, metamaskWallet } from "@egiftcard/react-native";
