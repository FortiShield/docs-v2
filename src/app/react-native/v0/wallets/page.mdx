import { Breadcrumb, createMetadata } from "@doc";
import { WalletCard, WalletCardGrid } from "@/components/others/WalletCard";
// icons
import MetaMaskIcon from "@public/icons/wallets/metamask.svg";
import CoinbaseIcon from "@public/icons/wallets/coinbase.svg";
import WalletConnectIcon from "@public/icons/wallets/walletconnect.svg";
import SmartWalletIcon from "@public/icons/wallets/smartwallet.svg";
import EmbeddedWalletIcon from "@public/icons/wallets/embeddedwallet.svg";
import LocalWalletIcon from "@public/icons/wallets/localwallet.svg";
import MagicLinkIcon from "@public/icons/wallets/magiclink.svg";
import RainbowIcon from "@public/icons/wallets/rainbow.svg";

export const metadata = createMetadata({
	image: {
		title: "Supported Wallets in egiftcard React Native SDK",
		icon: "react",
	},
	title: "Supported wallets | egiftcard React Native SDK",
	description: "List of supported wallets for egiftcard React Native SDK",
});

<Breadcrumb
	crumbs={[
		{
			name: "React Native",
			href: "/react-native/v0",
		},
		{
			name: "Wallets",
			href: "/react-native/v0/wallets",
		},
	]}
/>

# Wallets

egiftcard React Naive SDK comes out of the box with support for below listed wallets:

You can also easily integrate _any_ other wallet provider by a [Building a wallet interface](/wallet-sdk/v2/build) for it.

<WalletCardGrid>
	<WalletCard
		label="MetaMask"
		href="/react-native/v0/wallets/metamask"
		icon={MetaMaskIcon}
	/>
	<WalletCard
		label="Coinbase"
		href="/react-native/v0/wallets/coinbase"
		icon={CoinbaseIcon}
	/>
	<WalletCard
		label="WalletConnect"
		href="/react-native/v0/wallets/walletconnect"
		icon={WalletConnectIcon}
	/>
	<WalletCard
		label="Smart Account (Account Abstraction)"
		href="/react-native/v0/wallets/smartwallet"
		icon={SmartWalletIcon}
	/>
	<WalletCard
		label="In-App Wallet"
		href="/react-native/v0/wallets/embedded-wallet"
		icon={EmbeddedWalletIcon}
	/>
	<WalletCard
		label="Local Wallet"
		href="/react-native/v0/wallets/local-wallet"
		icon={LocalWalletIcon}
	/>
	<WalletCard
		label="Magic Link"
		href="/react-native/v0/wallets/magiclink"
		icon={MagicLinkIcon}
	/>
	<WalletCard
		label="Rainbow"
		href="/react-native/v0/wallets/rainbow"
		icon={RainbowIcon}
	/>
</WalletCardGrid>
