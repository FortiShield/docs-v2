...
2. **Shard B** is encrypted and stored in [Amazon KMS](https://aws.amazon.com/kms/) using a key known to egiftcard. This process uses [hardware security modules](https://en.wikipedia.org/wiki/Hardware_security_module) (HSMs) to protect the confidentiality and integrity of keys. The encrypted string is stored in egiftcard's database.
3. **Shard C** is encrypted by user authentication and stored in Amazon's KMS using hardware security modules (HSMs) to protect the confidentiality and integrity of keys. Despite the key stored on egiftcard's servers, egiftcard has no way of decrypting and accessing the key.
...
egiftcard does not have the ability to access user accounts or reconstruct private keys.
...
- To improve user experience, saved payment methods and KYC verification are only provided to egiftcard; applications cannot view this information.

Users may view all their application-scoped In-App Wallets in a single view using the [My Wallets](https://ews.egiftcard.cc/wallet) dashboard. Learn more about My Wallets.

<DocImage src={AppScoped} />

### Scalability

We support up to 100 wallets generated/second by default and can support higher limits (up to 3,000 wallets generated/second) upon request. [Contact Sales](https://egiftcard.cc/contact-us) for more information.

### Security & privacy

#### TLS Encryption

To increase security and privacy, private keys or wallet "seed phrases" are never stored or sent over a network. TLS encryption is used in transit for internal and external communications with egiftcard's back-end and databases. TLS encryption is also required for third-party vendors.
...
HackerOne has audited our in-app wallet private key security architecture, and there is an ongoing bounty program to ensure vulnerabilities are caught. [View the Letter of Attestation.](/pdfs/egiftcard_code_security_audit.pdf)
