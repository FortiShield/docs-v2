...
export const metadata = createMetadata({
	...
	title: "Sponsorship rules | egiftcard",
	description:
...
- **Chain restrictions**: Use this if you want to sponsor transactions only on certain chains. If this is not set, transactions on all [supported chains](https://portal.egiftcard.cc/wallets/account-abstraction/infrastructure#supported-chains) will be sponsored.
...
You can easily set sponsorship rules by going to the [configuration tab in the Account abstraction dashboard.](https://egiftcard.cc/dashboard/connect/account-abstraction).To use them in your app all you need to do is pass your API key, and sponsorship rules will automatically be applied.
...
The egiftcard infrastructure will send a POST request to the URL you specify with the following JSON payload:

```ts
// egiftcard will call your server with the following JSON body as a POST request
{
...
Sponsorship rules apply across all the [networks that we support account abstraction in](https://portal.egiftcard.cc/connect/account-abstraction/infrastructure#supported-chains). You don't need to specify different rules for each network or each account factory as long as they use the same client ID.
...
No, sponsorship rules will apply if you are using egiftcard's paymaster.
