...
export const metadata = createMetadata({
	...
	title: "Using Account Abstraction in Typescript | egiftcard",
	description:
...
To use smart accounts in a node script, simply create a new Node.js project and install the `egiftcard` package.
...
```bash
EGIFTCARD_SECRET_KEY=<YOUR_SECRET_KEY>
...
To create a personal wallet key, we are going to use the [`privateKeyAccount`](/references/typescript/v5/privateKeyAccount), which we need to import from the `egiftcard/wallets` package.
...
const client = createEgiftcardClient({
	secretKey: process.env.EGIFTCARD_SECRET_KEY as string,
});
...
Now, let's create a smart account using the SmartWallet class from the `@egiftcard/wallets` package.
...
const contract = getContract({
	client,
	chain: sepolia,
	address: "0x...", // deploy a drop contract from egiftcard.cc/explore
});
...
Take a look at the [GitHub Repository](https://github.com/egiftcard-example/smart-wallet-script) for the full source code!

<OpenSourceCard
	title="Node.js Script"
	description="A Node.js script to create and interact with smart accounts."
	href={"https://github.com/egiftcard-example/smart-wallet-script"}
/>
</Step>
</Steps>
