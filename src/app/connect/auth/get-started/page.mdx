...
export const metadata = createMetadata({
	image: {
		title: "Get Started with egiftcard Auth",
		icon: "wallets",
	},
	title: "Get started with Auth | egiftcard",
	description:
...
We can get started by cloning the example repository using the `npx egiftcard create` command:

```bash
npx egiftcard create --template egiftcard-auth-next
```

This will use the [Egiftcard CLI](/cli) to clone the example repository and install the necessary dependencies for us automatically.
...
NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN=example.com # your domain here
EGIFTCARD_AUTH_PRIVATE_KEY=... # your private key here
```
...
// Here we configure egiftcard auth with a domain and wallet
export const { EgiftcardAuthHandler, getUser } = EgiftcardAuth({
	domain: process.env.NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN || "",
...
// Use the EgiftcardAuthHandler as the default export to handle all requests to /api/auth/*
export default EgiftcardAuthHandler();
```

This file is a Next.js catch-all route, which will catch any requests to `/api/auth/*` endpoints. The `EgiftcardAuth` function takes in configuration for your Auth API. It has many optional parameters that aren't used here (you can read more about them on the [Auth Next.js page](/connect/auth/server-frameworks/next)), but the two mandatory parameters are `domain` and `wallet`.
...
The `EgiftcardAuth` function returns us a `EgiftcardAuthHandler`, which automatically sets up multiple authentication endpoints for us to use that handle logins, authentication, and logouts (as laid out in the [Auth API overview](/connect/auth/how-it-works/api)). It also returns a `getUser` function, which we can use on other endpoints to authenticate the user that sent a request.
...
First, let's look at the `pages/_app.tsx` file. Here, we want to setup the [`@egiftcard/react`](/react/latest) SDK to use with the Auth package. To do this, we need to wrap our app with the `EgiftcardProvider` component and add `authConfig` to it so that our frontend knows that we have an Auth backend setup.
