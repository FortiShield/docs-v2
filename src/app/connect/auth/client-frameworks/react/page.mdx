...
export const metadata = createMetadata({
	image: {
		title: "Auth in egiftcard React SDK",
		icon: "react",
	},
	title: "Auth in egiftcard React SDK",
...
Once you've set up an Auth API for your application, you can use the `@egiftcard/react` and `@egiftcard/auth` packages to add Auth support to your React apps.
...
```bash npm2yarn
npm install @egiftcard/react @egiftcard/sdk @egiftcard/auth ethers@5
```

Now we'll need to wrap our app with the `EgiftcardProvider` which stores the necessary context for Auth, and we'll provide some configuration with the `authConfig` option so that our frontend knows how to communicate with our Auth API.
...
The simplest way to add an Auth flow to our app is to use the `ConnectWallet` button. This button detects the `authConfig` on our `EgiftcardProvider` and will automatically prompt the user to connect their wallet with MetaMask, Coinbase Wallet, or WalletConnect, and then login to our app with Auth.

```tsx title="pages/index.tsx"
import { ConnectWallet } from "@egiftcard/react";
...
For a more customized setup with custom UI or logic, we can instead use the wallet connection and Auth hooks provided by the `@egiftcard/react` package.
...
```tsx
import { useUser } from "@egiftcard/react";
...
```tsx
import { useLogin, useLogout } from "@egiftcard/react";
...
```tsx
import { useLogin } from "@egiftcard/react";
...
```ts
import { useSwitchAccount } from "@egiftcard/react";
