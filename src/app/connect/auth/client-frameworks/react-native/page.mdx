...
export const metadata = createMetadata({
	image: {
		title: "Auth in egiftcard React Native SDK",
		icon: "react",
	},
	title: "Auth in egiftcard React Native SDK",
...
Now we'll need to wrap our app with the `EgiftcardProvider` which stores the necessary context for Auth, and we'll provide some configuration with the `authConfig` option so that our app knows how to communicate with our Auth API.
...
The simplest way to add an Auth flow to our app is to use the `ConnectWallet` button. This button prompts the user to connect to the `supportedWallets` specified in the `EgiftcardProvider`.
...
```tsx
import { ConnectWallet, useLogin } from "@egiftcard/react-native";
...
For a more customized setup with custom UI or logic, we can instead use the wallet connection and Auth hooks provided by the `@egiftcard/react-native` package.
...
```tsx
import { useUser } from "@egiftcard/react-native";
...
```tsx
import { useLogin, useLogout } from "@egiftcard/react-native";
...
```tsx
import { useLogin } from "@egiftcard/react-native";
