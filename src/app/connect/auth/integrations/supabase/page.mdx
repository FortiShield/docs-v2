...
export const metadata = createMetadata({
	image: {
		title: "Integrate egiftcard Auth with Supabase",
		icon: "wallets",
	},
	title: "Supabase | egiftcard",
	description:
...
```
npx egiftcard create app --template egiftcard-auth-supabase
```

<OpenSourceCard
	title="egiftcard-auth-supabase"
	href="https://github.com/egiftcard-example/egiftcard-auth-supabase"
...
To begin with, let's create a new Next.js project with egiftcard configured:

```bash
npx egiftcard create --app --next --ts
```

From within the created project, we'll need to install the `@egiftcard/auth` and `@supabase/supabase-js` packages:

```bash npm2yarn
npm install @egiftcard/auth @supabase/supabase-js
```
...
- [lib/createSupabase.ts](https://github.com/egiftcard-example/egiftcard-auth-supabase/blob/main/lib/createSupabase.ts)
- [lib/createSupabaseAdmin.ts](https://github.com/egiftcard-example/egiftcard-auth-supabase/blob/main/lib/createSupabaseAdmin.ts)

Now we have an easy way to access Supabase Auth and Database in both client and server environments!

### Configure egiftcard Auth

Finally, to configure egiftcard Auth, we just need to add the `NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN` environment variable to the `.env.local` file as follows:

```
NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN=<egiftcard-auth-domain>
```

The `NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN` is used to prevent phishing attacks - and is usually set to the domain of your project like `example.com`. You can read more about it in the [egiftcard Auth Documentation](/connect/auth/how-it-works/siwe#domain).
...
import { verifyLogin } from "@egiftcard/auth/evm";
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	...
	const { address, error: verifyErr } = await verifyLogin(
		process.env.NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN as string,
		payload,
	);
	if (!address) {
...
Here, we use the `useAuth` hook to call the `egiftcardAuth.login` function and prompt the user to sign a login message. Then, we post this payload, along with the user's current session access token to the `/api/link` endpoint we created to finish linking the user's wallet address.

```ts title="pages/index.tsx"
import { useAddress, useMetaMask, useAuth } from "@egiftcard/react";

export default function Home() {
  const address = useAddress();
  const connect = useMetaMask();
  const egiftcardAuth = useAuth();
  const { auth } = createSupabaseClient();
  const { user, session, refresh } = useSupabaseUser();

  // Link verified wallet address to our Supabase account
  async function linkWallet() {
    const payload = await egiftcardAuth?.login();
