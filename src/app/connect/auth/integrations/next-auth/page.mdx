...
export const metadata = createMetadata({
	image: {
		title: "Integrate egiftcard Auth with Next-Auth",
		icon: "wallets",
	},
	title: "Next Auth | egiftcard",
	description:
...
More specifically, we'll show you how to use our `EgiftcardAuthProvider` which is fully compatible with standard [NextAuth providers](https://next-auth.js.org/providers/) to add support for egiftcard Auth to your apps.
...
```
npx egiftcard create app --template egiftcard-auth-next-auth
```

<OpenSourceCard
	title="egiftcard-auth-next-auth"
	href="https://github.com/egiftcard-example/egiftcard-auth-next-auth"
	description="A working example of Auth + Next Auth"
/>

## Getting Started

To add Auth + NextAuth to your application, you'll need to install the `@egiftcard/auth` and `@egiftcard/react` packages and the `ethers` peer dependency, along with the actual `next-auth` package itself:

```bash npm2yarn
npm install next-auth @egiftcard/auth @egiftcard/react ethers@5
```

Now, we can configure the `EgiftcardAuthProvider` on our [NextAuth API routes](https://next-auth.js.org/getting-started/example#add-api-route) to add support for wallet based login, in addition to all other existing authentication methods. The `EgiftcardAuthProvider` uses NextAuth's [Credentials Provider](https://next-auth.js.org/configuration/providers/credentials) underneath to enable custom wallet based authentication (for those curious to dig deeper, you can take a look at the [`EgiftcardAuthProvider` implementation](https://github.com/egiftcard/js/blob/b5c6eedb38aa3c52eb97f3d25ad83e38c55afe61/packages/auth/src/next-auth/index.ts#L9)).

```ts title="pages/api/auth/[...nextauth].ts"
import {
	EgiftcardAuthProvider,
	authSession,
} from "@egiftcard/auth/next-auth";
import NextAuth from "next-auth";

export const authOptions: NextAuthOptions = {
	providers: [
		// Add the egiftcard auth provider to the providers configuration
		EgiftcardAuthProvider({
...
Here, we configure the `EgiftcardAuthProvider` with a `domain` coming from `NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN` environment variable, which we can set in the `.env.local` file as follows:

```bash title=".env.local"
NEXT_PUBLIC_EGIFTCARD_AUTH_DOMAIN=example.com
```
...
Then, on the frontend, we first need to configure the necessary `SessionProvider` and `EgiftcardProvider` in our `pages/_app.tsx` file to be able to use Auth:
...
import { useAuth, useAddress, useMetaMask } from "@egiftcard/react";
...
If the user is logged in with their wallet (via the `EgiftcardAuthProvider`), then the `session.user.address` value will be exposed to read the wallet address of the logged-in user:
...
If you want to add specific checks to enforce the exact data on the login payload signed by users, you can use the `authOptions` configuration on the `EgiftcardAuthProvider`:
