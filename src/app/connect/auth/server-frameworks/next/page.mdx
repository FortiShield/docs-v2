...
export const metadata = createMetadata({
	image: {
		title: "Integrate egiftcard Auth with Next.js",
		icon: "react",
	},
	title: "Auth in Next.js with egiftcard SDK",
...
```
npx egiftcard create app --template egiftcard-auth-next
```

<OpenSourceCard
	title="egiftcard-auth-next"
	href="https://github.com/egiftcard-example/egiftcard-auth-next"
	description="Auth Next example repository."
/>

## Getting Started

To add support for Auth to your Next backend, you'll need to install the `@egiftcard/auth` package (as well as any other packages you'll need for your [wallet configuration](/connect/auth/wallet-configuration), we'll need the `ethers` package for this example as we'll be using the `PrivateKeyWallet`):

```bash npm2yarn
npm install @egiftcard/auth ethers@5
```
...
We can set up the server with the `pages` directory by using the `@egiftcard/auth/next` entry point. First, we need to setup our [Auth API](/connect/auth/how-it-works/api) by creating a new file called `pages/api/auth/[...egiftcard].ts`:

```ts title="pages/api/auth/[...egiftcard].ts"
import { EgiftcardAuth } from "@egiftcard/auth/next";
import { PrivateKeyWallet } from "@egiftcard/auth/evm";

export const { EgiftcardAuthHandler, getUser } = EgiftcardAuth({
	domain: "example.com",
	wallet: new PrivateKeyWallet(process.env.EGIFTCARD_AUTH_PRIVATE_KEY || ""),
	egiftcardAuthOptions: {
		secretKey: "...",
	},
});

export default EgiftcardAuthHandler();
```

Here, we first configure our `EgiftcardAuth` instance with the `domain` of our app used for anti-phishing, as well as the admin wallet used to issue and verify JWTs. In this case, we use the `PrivateKeyWallet` from the `@egiftcard/auth/evm` entry point, but there are several other supported wallet setups, which you can learn more about on the wallet configuration page:
...
Finally, we export the `EgiftcardAuthHandler` which sets up all the `/api/auth/*` routes for us automatically. This is all the setup we need to set up a full Auth API on our Next.js backend!

### App Router Setup

Alternatively, we can set up auth using the app router. We can set up our [Auth API](/auth/how-auth-works/auth-api) by creating a new file called `app/api/auth/[...egiftcard]/route.ts` as follows:

Create a separate file named `egiftcardAuth.ts`:

```ts title="app/api/auth/[...egiftcard]/egiftcardAuth.ts"
import { EgiftcardAuthAppRouter } from "@egiftcard/auth/next";
import { PrivateKeyWallet } from "@egiftcard/auth/evm";

export const { EgiftcardAuthHandler, getUser } = EgiftcardAuthAppRouter({
	domain: "example.com",
	wallet: new PrivateKeyWallet(process.env.EGIFTCARD_AUTH_PRIVATE_KEY || ""),
	egiftcardAuthOptions: {
		secretKey: "...",
...
Import `egiftcardAuth.ts` in your `app/api/auth/[...egiftcard]/route.ts` file:

```ts title="app/api/auth/[...egiftcard]/route.ts"
import { EgiftcardAuthHandler } from "./egiftcardAuth";

export { EgiftcardAuthHandler as GET, EgiftcardAuthHandler as POST };
```

Like above, we first configure our `EgiftcardAuth` instance with the `domain` of our app used for anti-phishing, as well as the admin wallet used to issue and verify JWTs. In this case, we use the `PrivateKeyWallet` from the `@egiftcard/auth/evm` entry point, but there are several other supported wallet setups, which you can learn more about on the wallet configuration page:
...
import { getUser } from "./auth/[...egiftcard]";
...
```ts title="pages/api/auth/[...egiftcard].ts"
import type { User } from "@egiftcard/auth/next";
