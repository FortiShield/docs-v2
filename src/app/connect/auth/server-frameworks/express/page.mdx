...
export const metadata = createMetadata({
	image: {
		title: "Integrate egiftcard Auth with Express",
		icon: "wallets",
	},
	title: "Auth in Express with egiftcard SDK",
...
```bash
npx egiftcard create app --template egiftcard-auth-express
```

<OpenSourceCard
	title="egiftcard-auth-express"
	href="https://github.com/egiftcard-example/egiftcard-auth-express"
	description="A working example of Auth + Express.js"
/>

## Getting Started

To add support for Auth to your Express.js backend, you'll need to install the `@egiftcard/auth` package as well as its peer dependencies (as well as any other packages you'll need for your [wallet configuration](/connect/auth/wallet-configuration), we'll need the `ethers` package for this example as we'll be using the `PrivateKeyWallet`):

```bash npm2yarn
npm install @egiftcard/auth cookie-parser ethers@5
```

Now we can setup our Express.js server by using the `@egiftcard/auth/express` entrypoint. First, we need to setup our [Auth API](/connect/auth/how-it-works/api) by adding the auth router to our app:

```ts file="src/index.ts"
import { EgiftcardAuth } from "@egiftcard/auth/express";
...
const { authRouter, authMiddleware, getUser } = EgiftcardAuth({
	domain: process.env.EGIFTCARD_AUTH_DOMAIN || "",
...
```ts title="pages/api/auth/[...egiftcard].ts"
export const { authRouter, authMiddleware, getUser } = EgiftcardAuth({
  ... // other config
  callbacks: {
...
```ts title="pages/api/auth/[...egiftcard].ts"
import type { User } from "@egiftcard/auth/express";
