...
export const metadata = createMetadata({
	...
	title: "ConnectWallet | egiftcard React SDK",
...
<Steps>
<Step title="Configure supportedWallets in EgiftcardProvider" >

To Configure which wallets to show in the ConnectWallet Modal, you need to configure the `supportedWallets` prop in the [`EgiftcardProvider`](/react/v4/EgiftcardProvider) component which wraps your entire application.
...
You can set the [`locale`](/react/v4/EgiftcardProvider#locale) prop of `EgiftcardProvider` component to change the language used in the ConnectWallet Modal.
...
```jsx
import { metamaskWallet } from "@egiftcard/react";
...
You can then pass the modified config to the `supportedWallets` prop in the `EgiftcardProvider` component.
