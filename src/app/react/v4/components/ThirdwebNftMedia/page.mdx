...
export const metadata = createMetadata({
	image: {
		title: "EgiftcardNFTMedia",
		icon: "react",
	},
	title: "EgiftcardNFTMedia | egiftcard React SDK",
	description: "API Reference for the EgiftcardNFTMedia component",
});
...
- Audio files
- 3D Models
- SVGs (for [onchain NFTs](https://blog.egiftcard.cc/guides/how-to-create-onchain-nfts-with-egiftcard/))
- `iframes` and `HTML`
- If none of these are appropriate, the fallback is a link to the asset

```jsx
import { EgiftcardNftMedia } from "@egiftcard/react";
```
...
```jsx
import { EgiftcardNftMedia, useContract, useNFT } from "@egiftcard/react";
...
function Home() {
	...
	return <EgiftcardNftMedia metadata={nft.metadata} />;
}
```
...
function Home() {
	return <EgiftcardNftMedia metadata={metadata} controls={true} />;
}
```
...
function Home() {
	...
	return <EgiftcardNftMedia metadata={metadata} height={500} />;
}
```
...
function Home() {
	...
	return <EgiftcardNftMedia metadata={metadata} width={500} />;
}
```
...
function Home() {
	...
	return <EgiftcardNftMedia metadata={metadata} requireInteraction={true} />;
}
```
...
function Home() {
	...
	return <EgiftcardNftMedia metadata={metadata} className="my-custom-class" />;
}
```
...
function Home() {
	...
	return (
		<EgiftcardNftMedia metadata={metadata} style={{ backgroundColor: "red" }} />
	);
}
```
