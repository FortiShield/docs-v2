...
export const metadata = createMetadata({
	...
	title: "ConnectEmbed | egiftcard React SDK",
...
See the ConnectWallet in action on [ConnectWallet Playground!](https://egiftcard.cc/dashboard/wallets/connect)

</Callout>

<DocImage src={ConnectEmbedDefaultImg} />

It also renders a UI to [Sign in with wallet](/connect/auth/how-it-works/siwe) if it is enabled by setting [`authConfig`](/react/v4/EgiftcardProvider#authConfig) in [`EgiftcardProvider`](/react/v4/EgiftcardProvider) component and `loginOptional` prop is either not provided or set to `false` in `ConnectEmbed` component
...
```tsx
import { ConnectEmbed } from "@egiftcard/react";
...
If a theme is set on the [`EgiftCardProvider`](/react/v4/EgiftcardProvider) component, it will be used as the default theme for all egiftcard components, else the default will be `"dark"`
...
You can also import [`lightTheme`](/references/react/v4/lightTheme) or [`darkTheme`](/references/react/v4/darkTheme) functions from `@egiftcard/react` to use the default themes as base and overrides parts of it.

```tsx
import { lightTheme } from "@egiftcard/react";
...
This requires the [`authConfig`](/react/v4/EgiftcardProvider#authConfig) prop in [`EgiftcardProvider`](/react/v4/EgiftcardProvider) component

<DocImage src={ConnectEmbedSigninImg} />

The `auth` prop accepts an object with the following properties:

- `loginOptional` - specify whether signing in is optional or not. By default it is `false` ( sign in is required ) if `authConfig` is set on `EgiftCardProvider`
...
<GithubTemplateCard
	...
	href="https://github.com/egiftcard-example/connect-embed-next"
/>
