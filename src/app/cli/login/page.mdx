...
Authenticate your device and link it to your egiftcard account with the egiftcard CLI.
By logging in, you can access the egiftcard infrastructure and services which the CLI commands use under the hood such as storage.

```bash
npx egiftcard login [options]
```
...
1. Run the `login` command, this will open the dashboard to allow you to link your device to your egiftcard account.
...
```bash
npx egiftcard generate --key <secret-key>
```
